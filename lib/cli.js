...
CLI.prototype.bump = Promise.method(function () {
    ...
    .then(function () {
        if (!argv['git-push']) { return; }

        // gift won't let you do a default "git push" so we have to emulate the default behavior
        return repo.branch$()
        .then(function (branch) {
            return repo.config$()
            .then(function (config) {
                var remote = config.items['branch.'+branch.name+'.remote'] || 'origin';

                if (argv.commit) {
                    if (VERBOSE) { console.log("Pushing changes to %s/%s", remote, branch.name); }
                    return repo.remote_push$(remote, branch.name);
                } else {
                    if (VERBOSE) { console.log("Would push changes to %s/%s", remote, branch.name); }
                }
            });
        });
    })
    .then(function () {
        if (!argv['git-push'] || !argv.tag) { return; }
    ...
    .then(function () {
        ...
        if (argv.commit) {
            if (VERBOSE) { console.log("Publishing to NPM"); }

            return new Promise(function (resolve, reject) {
                var proc;
                if (VERBOSE) {
                    proc = cp.spawn('npm', [ 'publish' ], { stdio: 'inherit' });
                } else {
                    proc = cp.spawn('npm', [ 'publish' ]);
                }
            ...
            if (VERBOSE) { console.log("Would publish to NPM"); }
        }
    })
    .catch(function (err) {
